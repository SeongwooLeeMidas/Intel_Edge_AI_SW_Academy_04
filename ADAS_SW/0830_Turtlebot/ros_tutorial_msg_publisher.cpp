...
int main(int argc, char **argv)                         // 노드 메인 함수
{
  ...
  while (ros::ok())
  {
    ...
    ROS_INFO("sec = %d", msg.stamp.sec);   // ROS_INFO 라는 ROS 함수를 이용하여 count 변수를 표시한다
    ROS_INFO("nsec = %d", msg.stamp.nsec);   // ROS_INFO 라는 ROS 함수를 이용하여 count 변수를 표시한다
    ROS_INFO("str = %s", msg.str.c_str());   // ROS_INFO 라는 ROS 함수를 이용하여 count 변수를 표시한다
